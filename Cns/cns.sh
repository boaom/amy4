...
Config() {
	...
	clear
	echo $echo_e_arg "\033[0;34m以修改为本地国内源，请放心安装 \033[0m"
	echo ""
...
InstallFiles() {
	...
	$download_tool_cmd cns https://codeberg.org/asdf88/stn-28/raw/branch/master/Cns/linux_${machine}${softfloat} || Error "cns download failed."
	$download_tool_cmd cns.init https://codeberg.org/asdf88/stn-28/raw/branch/master/Cns/cns.init || Error "cns.init download failed."
	...
	if type systemctl && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/cns.service https://codeberg.org/asdf88/stn-28/raw/branch/master/Cns/cns.service || Error "cns.service download failed."
