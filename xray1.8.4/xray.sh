...
InstallFile() {
	...
	#install xray
	# Try GitHub Releases first, fallback to raw if not available
	binary_name="xray-${xray_UPX:+${xray_UPX}-}linux-${machine}${softfloat}"
	$download_tool_cmd xray "https://github.com/boaom/amy4/releases/latest/download/${binary_name}" || \
	$download_tool_cmd xray "https://raw.githubusercontent.com/boaom/amy4/main/xray/${xray_UPX}/linux_${machine}${softfloat}" || Error "xray download failed."
	$download_tool_cmd xray.init https://raw.githubusercontent.com/boaom/amy4/main/xray/xray.init || Error "xray.init download failed."
	...
	if which systemctl && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/xray.service https://raw.githubusercontent.com/boaom/amy4/main/xray/xray.service || Error "xray.service download failed."
