...
InstallFiles() {
	...
	binary_name="cns-${cns_UPX:+${cns_UPX}-}linux-${machine}${softfloat}"
	$download_tool_cmd cns "https://github.com/boaom/amy4/releases/latest/download/${binary_name}" || \
	$download_tool_cmd cns "https://raw.githubusercontent.com/boaom/amy4/main/cns/${cns_UPX}/linux_${machine}${softfloat}" || Error "cns download failed."
	$download_tool_cmd cns.init https://raw.githubusercontent.com/boaom/amy4/main/cns/cns.init || Error "cns.init download failed."
	...
	if type systemctl && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/cns.service https://raw.githubusercontent.com/boaom/amy4/main/cns/cns.service || Error "cns.service download failed."
